...
interface AuthContextType {
  user: User | null;
  token: string | null;
  // 登录返回结构增加 requiresOTPSetup、qrCodeURL、otpSecret，用于未完成OTP的补救流程
  login: (email: string, password: string) => Promise<{ success: boolean; message?: string; userID?: string; requiresOTP?: boolean; requiresOTPSetup?: boolean; qrCodeURL?: string; otpSecret?: string }>;
  ...
  const login = async (email: string, password: string) => {
    try {
      ...
      if (response.ok) {
        ...
        // 账户未完成OTP设置：后端返回401并携带 requires_otp_setup、user_id、qr_code_url、otp_secret
        if (data && data.requires_otp_setup && data.user_id) {
          return {
            success: true,
            userID: data.user_id,
